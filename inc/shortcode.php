    ...
    if (is_user_logged_in()) {
        ...
        if ($comment_count > 0) {
            return '<div class="privilege-content-wrapper comment-required">
                        <div class="privilege-indicator"><i class="fa-solid fa-lock"></i>评论可见</div>' . do_shortcode($content) . '</div>';
        }
    } else {
...
add_shortcode('comment', 'comment_visible_shortcode');
...
function logged_in_visible_shortcode($atts, $content = null) {
    ...
    if (is_user_logged_in()) {
        return '<div class="privilege-content-wrapper login-required">
                    <div class="privilege-indicator"><i class="fa-solid fa-lock"></i>登录可见</div>' . do_shortcode($content) . '</div>';
    } else {
        return '<div class="privilege-content-wrapper login-required">
...
add_shortcode('login', 'logged_in_visible_shortcode');
...
function vip_visible_shortcode($atts, $content = null) {
    ...
    if (!empty($atts['role'])) {
        ...
        if (current_user_can('read_vip_content') || in_array('vip', (array) $user->roles) || in_array('administrator', (array) $user->roles)) {
            return '<div class="privilege-content-wrapper vip-required">
                        <div class="privilege-indicator">VIP可见</div>' . do_shortcode($content) . '</div>';
        }
    }
...
add_shortcode('vip', 'vip_visible_shortcode');


function custom_clean_excerpt($content) {
    // 移除短代码及其内容，替换为提示
    $content = preg_replace('/\[login\].*?\[\/login\]/s', '[部分内容仅限登录用户查看]', $content);
    $content = preg_replace('/\[vip\].*?\[\/vip\]/s', '[部分内容仅限VIP用户查看]', $content);
    $content = preg_replace('/\[comment\].*?\[\/comment\]/s', '[部分内容评论后查看]', $content);
...
add_filter('get_the_excerpt', 'custom_clean_excerpt', 5);
