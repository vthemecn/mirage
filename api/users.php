...
class Users
{
    public function update($request)
    {
        ...
        if ($current_user_id != $user_id && !current_user_can('manage_options')) {
            return new \WP_Error('unauthorized', __('No permission to modify this user', 'vt'), array('status' => 401));
        }

        // 验证用户ID
        $user = get_user_by('id', $user_id);
        if (!$user) {
            return new \WP_Error('user_not_found', __('User does not exist', 'vt'), array('status' => 404));
        }
